...
提供与操作系统相关的，包括常用路径操作、进程管理、环境参数设置等几类（课程介绍三大类）

## 路径操作

os.path 子库，处理文件路径及信息

以 path 为入口： 目录或包含文件名称的文件的路径
...
如 os.path.abspath('file.txt')

### os.path.normpath(path)

归一化 path 的表示形式，统一用 两个斜杠 分隔路径，便于字符串处理
...
组合 path 与 paths（其他路径），返回一个路径字符串

```python
os.path.join('D://download//', 'file.txt')
>>> 'D://download//file.txt'
```
...
access，访问，返回 path 对应文件或目录上一次的访问时间

### os.path.getmtime(path)

modify，修改，返回 path 对应文件或目录最近一次的修改时间

### os.path.getctime(path)

create，创建，返回 path 对应文件或目录的创建时间
...
具体几轮比较才能结束不知道 -> while 循环

random 函数生成随机变量，在A能力内，A获得分数
...
具体包括： 主程序、子程序、子程序之间关系

分而治之： 一种分而治之、分层抽象、体系化的设计思想

在分块过程中，注意耦合

功能之间有很多交流，不能独立存在 -> 选则紧耦合 -> 尽量不要单独成块

功能之间只有少数的数据交互 -> 选择松耦合 -> 单独成块更方便

#### 配置化设计

如何把程序中的数据和程序分离

对一个程序来说，功能部分 -> 程序引擎， 具体的实现（如轨迹）-> 数据或配置文件形式体现

引擎 + 配置： 程序执行和配置分离，将可选参数配置化 -> 程序开发变成配置文件编写，扩展功能而不修改程序

关键在于接口设计，要求清晰明了，灵活可扩展
...
数据流： 应用中的数据流动怎么形成的，数据流是不是有效、高效

模块化： 怎么划分、模块之间的结构是否合理

体系架构： 整体的体系结构是不是冗余，是否可靠

##### 设计与实现

结合架构完成关键设计及系统实现

考虑设计是否可扩展， 是否灵活，是否能适应未来变化，对应进行优化设计，使整体功能实现合理、高效
...
PyPI： Python Package Index， Python包索引，由PSF（Python Software Foundation）来维护，用来展示全球 Python 计算生态的主站
...
pip list： 列出当前系统已经安装的

### 集成安装

结合特定安装工具，批量安装第三方库

如 Anaconda http://www.continuum.io

### 文件安装

有些第三方库pip下载后，需要编译再安装（因为是源代码），如果操作系统没有编译环境，则能下载不能安装
